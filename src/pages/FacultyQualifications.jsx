...
import { FacultyQualificationStat } from '@/api/mockEntities';
...
export default function FacultyQualificationsPage() {
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        const data = await FacultyQualificationStat.list();
  ...
  return (
    <div dir="rtl" className="p-4 md:p-8 space-y-8">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-bold text-white">مؤهلات أعضاء هيئة التدريس لعام 1444هـ</h1>
        <p className="text-blue-200 mt-1">توزيع المؤهلات العلمية لأعضاء هيئة التدريس حسب الجنسية والجنس</p>
      </motion.div>
      ...
      <Card className="bg-white/95 backdrop-blur-sm border-white/30 shadow-lg p-6">
          <h3 className="text-xl font-bold text-gray-800 mb-4">توزيع المؤهلات حسب الجنسية</h3>
      ...
      <Card className="bg-white/95 backdrop-blur-sm border-white/30 shadow-lg p-6">
        <h3 className="text-xl font-bold text-gray-800 mb-4">التفاصيل الكاملة</h3>
